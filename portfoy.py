...
def smart_parse(text_val):
    ...
    # Virgül varsa noktaya çevir (Avrupa/TR -> US)
    # Örn: 30,26 -> 30.26
    if "," in val:
        val = val.replace(",", ".")
    
    # Eğer birden fazla nokta varsa (1.000.50 gibi), sonuncusu hariç sil
    if val.count(".") > 1:
        parts = val.split(".")
...
st.markdown(f"""<div class="ticker-container market-ticker">{market_html}</div><div class="ticker-container portfolio-ticker">{portfolio_html}</div>""", unsafe_allow_html=True)
...
# --- HESAPLAMA (BU SEFER MANUEL FALLBACK YOK - HATAYI GÖRELİM) ---
        ...
        try:
            if pazar == "FON":
                ...
                if not hist.empty:
                    ...
                    # VERİ YOKSA 0 OLSUN Kİ HATAYI GÖRELİM (Maliyet'e eşitleme!)
                    curr_price = 0 
                    prev_close = 0
        except: 
            curr_price = 0
