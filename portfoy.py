...
# Fon kategorileri (örnek; istersen kendine göre düzenlersin)
FON_CATEGORY_MAP = {
    "TTE": "Altın Fon",
    "MAC": "Altın Fon",
    "AFT": "Karma Fon",
...
def run_analysis(df, usd_try_rate, view_currency):
    ...
    for i, row in df.iterrows():
        ...
        # 100x şişmiş maliyeti otomatik düzelt
...
st.divider()
    ...
    if filter_text == "FON":
        ...
        st.subheader("📊 Fon Kategorileri (Portföy Değeri Bazında)")
        df_fon = df_filtered.copy()
        df_fon["Kategori"] = df_fon["Kod"].map(FON_CATEGORY_MAP).fillna("Diğer")
        cat_group = df_fon.groupby("Kategori")["Değer"].sum().reset_index()
        fig_cat = px.pie(cat_group, values="Değer", names="Kategori", hole=0.4)
        st.plotly_chart(fig_cat, use_container_width=True)

        # 2) Fon karşılaştırma (TEFAS tarihsel)
        st.subheader("📈 Fon Karşılaştırma (TEFAS Verisi ile)")
        fon_list = df_filtered["Kod"].unique().tolist()
        secilen_fonlar = st.multiselect(
            "Karşılaştırmak istediğin fonları seç (max 6):",
            fon_list,
            max_selections=6
        )
        if secilen_fonlar:
            comp_df = pd.DataFrame()
            for code in secilen_fonlar:
                h = get_tefas_history(code, days=365)
                if h is None or h.empty:
                    continue
                prices = pd.to_numeric(h["Price"], errors="coerce").dropna()
                if prices.empty:
                    continue
            ...
            if not comp_df.empty:
                ...
                fig_comp.update_layout(
                    title="Seçili Fonların 1Y Göreli Performansı (Başlangıç = 100)",
                    xaxis_title="Tarih",
                    yaxis_title="Endeks (100=Başlangıç)",
                    template="plotly_dark",
                    height=500,
                    legend_title_text="Fon"
                )
                st.plotly_chart(fig_comp, use_container_width=True)
            else:
                st.info("Seçilen fonlar için yeterli TEFAS verisi bulunamadı.")

        # 3) Risk tablosu
        st.subheader("📉 Fon Risk Tablosu (1Y)")
        risk_rows = []
        for code in fon_list:
            total_ret, vol_annual, max_dd = calc_fund_risk(code, days=365)
            risk_rows.append({
                "Fon": code,
                "1Y Getiri": total_ret,
                "Yıllık Volatilite": vol_annual,
                "Maks. Düşüş": max_dd
            })
        if risk_rows:
            risk_df = pd.DataFrame(risk_rows)
            risk_df = risk_df.sort_values(by="1Y Getiri", ascending=False)
            st.dataframe(
    ...
    # Diğer pazarlar için tarihsel simülasyon
        ...
        if not portfoy_df.empty:
            ...
            with st.form("sell_asset_form"):
                ...
                if satilacak_kod:
                    ...
                    m_adet, m_maliyet, pazar_yeri = 0, 0, ""
